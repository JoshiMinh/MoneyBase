...
class _AuthCardState extends State<AuthCard> {
  ...
  Future<void> _applyPersistence(bool keepSignedIn) async {
    if (!kIsWeb) {
      return;
    }
    try {
      ...
      // Persistence configuration is only supported on the web target.
    }
  }
...
class _GoogleLogoPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final strokeWidth = dimension * 0.24;
    final offset = (size.width - dimension) / 2;
    final rect = Rect.fromLTWH(
      offset + strokeWidth / 2,
      (size.height - dimension) / 2 + strokeWidth / 2,
    ...
    void drawSegment(double startAngle, double sweepAngle, Color color) {
      final paint = basePaint..color = color;
      canvas.drawArc(rect, startAngle, sweepAngle, false, paint);
    }

    drawSegment(-3 * math.pi / 4, math.pi / 2, const Color(0xFF4285F4));
    drawSegment(-math.pi / 4, math.pi / 2, const Color(0xFFDB4437));
    drawSegment(math.pi / 4, math.pi / 2, const Color(0xFFF4B400));
    drawSegment(3 * math.pi / 4, math.pi / 2, const Color(0xFF0F9D58));
    ...
    final barStart = Offset(rect.center.dx, barY);
    final barEnd = Offset(rect.right, barY);
